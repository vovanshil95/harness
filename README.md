...
Каждому — свой бот, свой контейнер агента, свой WebDAV и свой волт.

| | пользователь 1 | пользователь 2 |
|---|---|---|
| агент | `hx-hermes` | `hx-hermes2` |
| WebDAV | `hx-webdav` | `hx-webdav2` |
| адрес WebDAV | `.../webdav` | `.../webdav2` |
| каталог | `webdav-data/` | `webdav2-data/` |
| данные агента | `hermes-data/` | `hermes2-data/` |
| токен бота | `TELEGRAM_BOT_TOKEN` | `TELEGRAM_BOT_TOKEN2` |

Переключение модели второму: `HX=2 ./switch-model.sh kimi`

**Почему два бота, а не один.** Telegram позволяет опрашивать токен только одному
процессу: второй получает `409 Conflict: terminated by other getUpdates request`.
Документация Hermes подтверждает — «the same token cannot be polled by two profiles
at once». Вебхуки не спасают: адаптер работает только опросом и при старте сам
вызывает `delete_webhook`.

**Почему два контейнера, а не два профиля.** Профили Hermes (`multiplex_profiles`)
разделяют конфиг, память и сессии, но не файловую систему — изоляция рабочих
каталогов пока лишь [запрошенная функция](https://github.com/NousResearch/hermes-agent/issues/9514).
Отдельные контейнеры дают гарантию через монтирование: чужой каталог физически
не подключён.

Проверено: `hermes2` видит в `/opt/data/vault` пустой каталог, до волта первого
не достаёт, пароли обоих WebDAV дают `401` на чужом адресе.

**Чего это не даёт.** Владелец сервера имеет root и прочитает любой каталог через
SSH в обход схемы. Изоляция защищает от ошибок агента, а не от администратора машины.
